...
In some situations, MathJax may conflict with Markdown-it. For example, in `$x_i$` the underscore `_` may be interpreted by Markdown-it as the start of italics. To avoid this, you can, in this case, escape the underscore with a backslash: `$x\_i$`. Alternatively, you can use `*` or `<em>` instead of `_` for italics in Markdown. 
